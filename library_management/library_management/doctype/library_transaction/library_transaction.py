...
class LibraryTransaction(Document):
    def validate(self):
        print("check", self.article, self.transaction_date, self.name)
        last_transaction = frappe.get_list("Library Transaction",fields=["transaction_type", "transaction_date"],filters = {"article": self.article,"transaction_date": ("<=", self.transaction_date),"name": ("!=", self.name)})
